...
def plot_pass_fail_euler_diagram(sizes: list[int]) -> tuple[Figure, Axes]: 
    ...
    label_specs = [
        ...
        (old_pass_pos[0] - 0.5, old_pass_pos[1] - pass_radius - 0.15, "old_pass", 11, "normal", "italic", "#534AB7"),
        (new_pass_pos[0] + 0.5, new_pass_pos[1] - pass_radius - 0.15, "new_pass", 11, "normal", "italic", "#0F6E56"),
    ]
    
    for x, y, text, size, weight, style, colour in label_specs:
        ax.text(x, y, text, ha="center", va="bottom",
                fontsize=size, fontweight=weight, fontstyle=style, color=colour)
